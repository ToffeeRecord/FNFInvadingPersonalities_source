...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				case 'mistakes' | 'vanish' | 'woods' | 'Old-Mistakes':
					curStage = 'bedroom';
				case 'coladdiction' | 'happy' | 'Old-Coladdiction':
					curStage = 'bedroomCola';
				case 'twitching' | 'Old-Twitching':
					curStage = 'bedroomFilthy';
				case 'fluffiness' | 'Old-Fluffiness' | 'expurgation':
					curStage = 'bedroomAwu';
				case 'storm-screamer':
					curStage = 'turintoLol';
				default:
		...
		switch (curStage)
		{
			...
			case 'bedroom':
				if(PlayState.SONG.song == "Old-Mistakes")
				{
					var bg:BGSprite = new BGSprite('song1/1_BG_WithoutBed', 300, -100, 0.9, 0.9);
					bg.setGraphicSize(Std.int(bg.width * 2));
					add(bg);

					var bed:BGSprite = new BGSprite('song1/1_BG_Bed', 300, -100, 0.9, 0.9);
					bed.setGraphicSize(Std.int(bg.width * 2));
					add(bed);
				}
				else
				{
					var bg:BGSprite = new BGSprite('Song1_BG_Better', -600, -400, 0.9, 0.9);
					add(bg);

					gfGroup = new FlxSpriteGroup(300, 115);
				}
			
			case 'bedroomCola':
				if(PlayState.SONG.song == "Old-Coladdiction")
				{
					var bg:BGSprite = new BGSprite('song2/2_BG_WithoutBed', 300, -100, 0.9, 0.9);
					bg.setGraphicSize(Std.int(bg.width * 2));
					add(bg);

					var bed:BGSprite = new BGSprite('song2/2_BG_Bed', 300, -100, 0.9, 0.9);
					bed.setGraphicSize(Std.int(bg.width * 2));
					add(bed);
				}
				else
				{
					var bg:BGSprite = new BGSprite('Song2_BG_Better', -600, -400, 0.9, 0.9);
					add(bg);
			...
			case 'bedroomFilthy':
				if(PlayState.SONG.song == "Old-Twitching")
				{
					var bg:BGSprite = new BGSprite('song3/3_BG_WithoutBed', 300, -100, 0.9, 0.9);
					bg.setGraphicSize(Std.int(bg.width * 2));
					add(bg);

					var bed:BGSprite = new BGSprite('song3/3_BG_Bed', 300, -100, 0.9, 0.9);
					bed.setGraphicSize(Std.int(bg.width * 2));
					add(bed);
				}
				else
				{
					var bg:BGSprite = new BGSprite('Song3_BG_Better', -600, -400, 0.9, 0.9);
					add(bg);
			...
			case 'bedroomAwu':
				if(PlayState.SONG.song == "Old-Fluffiness")
				{
					var bg:BGSprite = new BGSprite('song4/4_BG_WithoutBed', 300, -100, 0.9, 0.9);
					bg.setGraphicSize(Std.int(bg.width * 2));
					add(bg);
				...
				{
					var bg:BGSprite = new BGSprite('Song4_BG_Better', -600, -400, 0.9, 0.9);
					add(bg);
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:Int = checkForAchievement([1, 2, 3, 4, 5, 7, 8, 9, 10]);
			if(achieve > -1) {
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if(daNote.noteType == 'Substance Note')
		{
			health -= daNote.oofmissHealth; //For testing purposes
			trace(daNote.oofmissHealth);
			songMisses++;
			vocals.volume = 0;
			RecalculateRating();
		}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(note.noteType == 'Cola Note') {
				PlayState.SONG.speed += note.oofhitHealth;
			}
		...
		for (i in 0...arrayIDs.length) {
			if(!Achievements.achievementsUnlocked[arrayIDs[i]][1]) {
				switch(arrayIDs[i]) {
					...
					case 3:
						if(PlayState.SONG.speed > 3.4 && !practiceMode && !cpuControlled)
						{
							Achievements.unlockAchievement(arrayIDs[i]);
							return arrayIDs[i];
						}
