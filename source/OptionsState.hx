...
class PreferencesSubstate extends MusicBeatSubstate
{
		...
		for (i in 0...options.length)
		{
			...
			if(!isCentered) {
				...
				if(useCheckbox) {
					...
					if(options[i] != 'Clear Achievements')
					{
			...
			else if(controls.ACCEPT && nextAccept <= 0)
			{
				switch(options[curSelected])
				{
					case 'Clear Achievements':
						...
						descText.alpha = 0;
						openSubState(new YouSureSubstate());
				}
			}
			else
			{
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Clear Achievements':
				daText = "If pressed, your achievements will be wiped away!\nSelf explanatory, huh?";
		}
...
class YouSureSubstate extends MusicBeatSubstate
{
	private static var curSelected:Int = 0;
	static var unselectableOptions:Array<String> = [
		'ARE YOU SURE?'
